...
class PartialMiniMaxPolicy(MiniMaxPolicy):
    ...
    def __init__(self, evaluator: Evaluator, sample_size_resolver: Callable[[int], int], depth=3):
        super().__init__(evaluator, depth)
        self.name = 'Partial MiniMax with depth {} and evaluator {}'.format(depth, evaluator.name)
