...
from minimax_policy.evaluator.Evaluator import Evaluator
...
class BidirectionalStepsWithWeightEval(Evaluator):
    ...
    def __init__(
        ...
        kill_cost=DEFAULT_KILL_COST,
        active_base_cost=DEFAULT_ACTIVE_BASE_COST,
        active_cell_cost=DEFAULT_ACTIVE_CELL_COST,
    ):
    ...
    def calc_all_active(self, active: Deque, team_value: int, game_state: GameState):
        ...
        while active:
            i = active.pop()
            for cell_i in game_state.get_cell_neighbours_indices(
                game_state.index_to_position(i)
            ):
    ...
    def evaluate(self, game_state: GameState) -> float:
        ...
        return self.calc_all_active(
            active_positions[1], 1, game_state
