...
def play_game_between_policies(
    policy1: EstimatingPolicy,
    policy2: EstimatingPolicy,
    h=9,
    w=9,
    show=False,
    show_steps=False,
    ...
    if show:
        ...
        if winning_team == Teams.BLUE:
            print(
                str_tmp.format(
                    policy1.name, CellStates.symbol(CellStates.BLUE_BASE), move_count
                )
            )
        else:
            print(
                str_tmp.format(
                    policy2.name, CellStates.symbol(CellStates.RED_BASE), move_count
                )
            )
        game.print_field()
...
def move(game: GameState, policy: EstimatingPolicy, show_steps: bool):
    ...
    if show_steps:
        game.print_field()
        print(
            "V = {}, policy {} checked {} positions to come up with that".format(
                value, policy.name, policy.pos_checked
            )
        )
        print()
...
def compare_policies(
    evaluated: EstimatingPolicy,
    compare_to: EstimatingPolicy,
    n,
    h,
    w,
    show=False,
    ...
    for _ in range(n):
        wins += (
            0
            if play_game_between_policies(compare_to, evaluated, h, w, show, show_steps)
            else 1
        )
        wins += play_game_between_policies(
            evaluated, compare_to, h, w, show, show_steps
        )

    print(
        "{} -- {}:{} -- {} evaluation took: {}".format(
            evaluated.name, wins, n * 2 - wins, compare_to.name, readable_time_since(t)
        )
    )
    return wins
...
if __name__ == "__main__":
    ...
    model_new_2gen.load_state_dict(
        torch.load("../rl/learning/data/model8-second-gen-conv-15-1.pt")
    )
    model_new_2gen.eval()
    model_2 = ModelBasedPolicy(model_new_2gen, PlainFeatureExtractor(), h, w, 0.1)

    model_new_6 = ConvolutionValue(h, w)
    model_new_6.load_state_dict(
        torch.load("../rl/learning/data/model8-conv-disc2-10iz10.pt")
    )
    model_new_6.eval()
    model_6 = ModelBasedPolicy(model_new_6, PlainFeatureExtractor(), h, w, 0.1)

    model_guided = ModelGuidedMiniMax(
        model_new_6,
    ...
    model_tree = ModelTreeD2Policy(
        model_new_6, PlainFeatureExtractor(), h, w, lambda x: 30, 0.1
    )
    model2_tree = ModelTreeD2Policy(
        model_new_2gen, PlainFeatureExtractor(), h, w, lambda x: 30, 0.1
    )
