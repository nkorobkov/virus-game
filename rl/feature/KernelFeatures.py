...
class KernelFeatureExtractor(FeatureExtractor):
    ...
    def get_horizontal_kernels(self):
        res = torch.unsqueeze(
            torch.tensor(PAIRS + REVERSED_PAIRS, requires_grad=False), 2
        )
        res.unsqueeze_(1)
        return res.float()
    ...
    def get_vertical_kernels(self):
        res = torch.unsqueeze(
            torch.tensor(PAIRS + REVERSED_PAIRS, requires_grad=False), 1
        )
        res.unsqueeze_(1)
        return res.float()
    ...
    def get_diagonal_kernels(self):
        ...
        for a, b in PAIRS + REVERSED_PAIRS:
            kernels.append([[0, a], [b, 0]])
            kernels.append([[a, 0], [0, b]])
    ...
    def get_features(self, games):
        ...
        kernel_features = (
            torch.cat(
                (
                    F.conv2d(fields, self.v_kernels).view(fields_count, -1),
                    F.conv2d(fields, self.h_kernels).view(fields_count, -1),
                    F.conv2d(fields, self.d_kernels).view(fields_count, -1),
                ),
                dim=1,
            )
            == 4
