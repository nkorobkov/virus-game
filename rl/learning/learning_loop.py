...
class Trainer:
    ...
    def training_loop(self):
        ...
        features, labels = self.data_sampler.sample_data_by_self_play_with_model(
            self.model, self.self_play_batch_size
        )
    ...
    def train_on_data(
        self, features, labels, epoch_count, print_every=50, eval_every=200
    ):
        ...
        for epoch in range(epoch_count):
            ...
            for i in range(0, train_features.shape[0] // self.minibatch_size):
                ...
                batch_values = train_game_values[
                    i * self.minibatch_size : (i + 1) * self.minibatch_size
                ]
            ...
            if (epoch + 1) % print_every == 0:
                ...
                print(
                    "epoch {}, loss {:.5}, test loss: {:.5}, time elapsed: {}".format(
                        epoch + 1, loss, test_loss, readable_time_since(t)
                    )
                )
    ...
    def evaluate_model(self, labels):
        ...
        model_policy = ModelBasedPolicy(
            self.model, self.data_sampler.feature_extractor, h, w, exploration=0.05
        )

        model_new_6_epoch = ConvolutionValue(h, w)
        model_new_6_epoch.load_state_dict(
            torch.load("data/model8-conv-disc2-10iz10.pt")
        )
        model_new_6_epoch.eval()
        model_6 = ModelBasedPolicy(
            model_new_6_epoch, PlainFeatureExtractor(), h, w, 0.05
        )
        compare_to = model_6
        ...
        print(
            "mse loss on mean over batch prediction  = {:.3}".format(
                default_prediction_loss
            )
        )
    ...
    def print_linear_weights_stat(self):
        ...
        for m in self.model.modules():
            if isinstance(m, torch.nn.Linear):
                print(
                    "Linear Layer {}, in: {}, out: {} max/mean/min/std weight: {:.3}/{:.3}/{:.3}/{:.3}".format(
                        ln,
                        m.in_features,
                        m.out_features,
                        m.weight.max(),
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        model,
        data_sampler,
        save_path="data/model8-second-gen-conv-{}.pt",
        minibatch_size=64,
        gamma=0.95,
    )
