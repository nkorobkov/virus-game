...
class LinearValue(torch.nn.Module):
    ...
    def init_weight_for_kernel_features(self):
        weight_for_move = torch.tensor([2.0])
        weights_for_plain_features_blue = torch.zeros(
            (self.plain_features_weights_count // 2)
        ).normal_(1, 0.1)
        weights_for_plain_features_red = torch.zeros(
            (self.plain_features_weights_count // 2)
        ).normal_(-1, 0.1)
        weights_for_kernel_features = torch.zeros(
            self.input_size - 1 - self.plain_features_weights_count
        ).normal_(0, 0.2)
        weights = torch.cat(
            (
                weight_for_move,
