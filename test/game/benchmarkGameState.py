...
def setup_for_field(field, h, w):
    setup = (
        ...
        + field
        + """     
game = GameState.from_field_list({},{},field,Teams.BLUE)""".format(
            h, w
        )
    )
    return setup
...
def test_field(field, h, w, mes=""):
    t = timeit.timeit(
        "game.get_all_moves()", setup=setup_for_field(field, h, w), number=1000
    )
...
cProfile.run(
    "timeit.timeit('game.get_all_moves()', setup=setup_for_field(fieldLayeredBigBases, 14,7), number=1000)"
)
