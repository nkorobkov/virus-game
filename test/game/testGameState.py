...
class TestUtils(unittest.TestCase):
    ...
    def testGetCellState(self):
        ...
        self.assertEqual(
            self.game.get_cell_state(Position(2, 3)), CellStates.RED_ACTIVE
        )
...
class TestNeighboursResolving(TestWithSimpleField):
    def testMiddleInd(self):
        pos = Position(5, 5)
        expected = {
            self.game.hw_to_index(4, 4),
            self.game.hw_to_index(4, 5),
            self.game.hw_to_index(4, 6),
            self.game.hw_to_index(5, 4),
            self.game.hw_to_index(5, 6),
            self.game.hw_to_index(6, 4),
            self.game.hw_to_index(6, 5),
            self.game.hw_to_index(6, 6),
        }
    ...
    def testCorner(self):
        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(9, 9))),
            {
                self.game.hw_to_index(8, 9),
                self.game.hw_to_index(8, 8),
                self.game.hw_to_index(9, 8),
            },
        )

        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(0, 0))),
            {
                self.game.hw_to_index(0, 1),
                self.game.hw_to_index(1, 1),
                self.game.hw_to_index(1, 0),
            },
        )

        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(0, 9))),
            {
                self.game.hw_to_index(0, 8),
                self.game.hw_to_index(1, 8),
                self.game.hw_to_index(1, 9),
            },
        )

        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(9, 0))),
            {
                self.game.hw_to_index(8, 0),
                self.game.hw_to_index(8, 1),
                self.game.hw_to_index(9, 1),
            },
        )

    def testSide(self):
        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(5, 9))),
            {
                self.game.hw_to_index(4, 9),
                self.game.hw_to_index(6, 9),
                self.game.hw_to_index(4, 8),
                self.game.hw_to_index(5, 8),
                self.game.hw_to_index(6, 8),
            },
        )

        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(5, 0))),
            {
                self.game.hw_to_index(4, 0),
                self.game.hw_to_index(6, 0),
                self.game.hw_to_index(4, 1),
                self.game.hw_to_index(5, 1),
                self.game.hw_to_index(6, 1),
            },
        )

        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(0, 5))),
            {
                self.game.hw_to_index(0, 4),
                self.game.hw_to_index(0, 6),
                self.game.hw_to_index(1, 4),
                self.game.hw_to_index(1, 5),
                self.game.hw_to_index(1, 6),
            },
        )

        self.assertSetEqual(
            set(self.game.get_cell_neighbours_indices(Position(9, 5))),
            {
                self.game.hw_to_index(9, 4),
                self.game.hw_to_index(9, 6),
                self.game.hw_to_index(8, 4),
                self.game.hw_to_index(8, 5),
                self.game.hw_to_index(8, 6),
            },
        )
...
class TestSingleMovesResolving(unittest.TestCase):
    def testElementary(self):
        ...
        expected: Mask = [False, True, False, True, True, False, False, False, False]
    ...
    def testBaseUsage(self):
        ...
        expected: Mask = [False, False, True, True, True, True, False, False, False]
        smm, _ = game.get_all_single_moves_mask()
    ...
    def testBaseInfluence(self):
        ...
        expected: Mask = [False, False, True, True, True, False, False, True, True]
        smm, _ = game.get_all_single_moves_mask()
    ...
    def testPossibleMoves(self):
        ...
        expected: Mask = [False, False, True, True, False, True, False, True, True]
        smm, _ = game.get_all_single_moves_mask()
    ...
    def testSubcomplexBlueMove(self):
        ...
        expected: Mask = [False, False, True, False, True, False, False, True, False]
        smm, _ = game.get_all_single_moves_mask()
...
class TestIntermediateStepsMovesResolving(unittest.TestCase):
    ...
    def testMaskToPositions(self):
        game = GameState(3, 3)
        mask: Mask = [True, False, False, True, False, False, False, False, True]
        positions = game.get_single_moves_positions_from_mask(mask)

        self.assertEqual(3, len(positions))
        self.assertSequenceEqual(
            [Position(0, 0), Position(1, 0), Position(2, 2)], positions
        )
    ...
    def testDoubleMovesResolving(self):
        ...
        (
            ...
            second_to_firsts,
            first_to_seconds,
        ) = game.get_all_double_moves_from_single_moves(
            single_positions, single_moves_mask, active_bases_seen
        )

        expected = [
            [Position(h=0, w=1), Position(h=1, w=2)],
    ...
    def testDoubleMovesResolving2(self):
        ...
        expected = [
            [Position(h=2, w=2), Position(h=0, w=1)],
    ...
    def test2StepMovesResolving(self):
        ...
        d_step_moves = list(
            game.get_all_ds_steps_moves(single_positions, second_to_firsts)
        )

        expected = [
            (Position(h=0, w=1), Position(h=1, w=1), Position(h=1, w=2)),
    ...
    def test3StepMovesResolving(self):
        ...
        t_step_moves = list(
            game.get_all_3_steps_moves(
                double_moves, single_moves_mask, active_bases_seen
            )
        )

        expected = [
            (Position(h=0, w=1), Position(h=1, w=2), Position(h=0, w=3)),
...
class TestMove(unittest.TestCase):
    def testElementary(self):
        ...
        expected_movable_masks: MaskDict = {
